...
Version 3.0: Multi-User Support with SQLite Persistent Storage
Features: User tracking, progress persistence, admin dashboard, real-time data storage
"""
...
from utils.database import (
    ...
    get_user_weak_areas, export_user_data_csv
)
...
def show_topic_select():
    """Topic selection"""
    st.title(f"📖 Topic Mastery Mode | {st.session_state.user_name}")
    st.write("Master each topic before the mock exam! (MCQ Questions Only)")
    ...
    for idx, (name, key) in enumerate(topics.items()):
        with cols[idx]:
            ...
            if st.button(f"{name}\n\n({q_count} MCQs)", use_container_width=True, key=f"topic_{key}"):
